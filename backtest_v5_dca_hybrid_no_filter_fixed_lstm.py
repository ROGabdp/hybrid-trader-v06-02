...
"""
================================================================================
V5 DCA + AI Hybrid 無濾網 Fixed LSTM 回測
(No Filter + Fixed LSTM Backtest)
================================================================================
結合 DCA Hybrid 策略與固定 LSTM 模型功能。
使用 V5 模型（對稱獎勵訓練），無唐其安濾網限制。

**V5 模型特點**:
- Buy Agent 使用對稱獎勵結構訓練
- 無 Donchian 濾網限制，AI 可自由判斷買入時機
- Sell Agent 與 V4 相同

**Fixed LSTM 特點**:
- 首次執行時訓練 LSTM 並儲存為 _fixed 版本
- 後續執行會自動偵測並使用現有的 _fixed 模型
- 保證每次回測使用相同的 LSTM，結果完全一致
...
class LeveragedSharedPoolBacktester:
    ...
    def run(self, df: pd.DataFrame, feature_cols: list) -> dict:
        ...
        # V5 無濾網模式 - 不需要讀取 Signal_Buy_Filter
        
        # =========== 動態資金分配 ===========
        # yearly_pool: 當年度可用資金池 (包含年初注入 + AI 賣出回收)
        # 每月買入金額 = yearly_pool / 剩餘月數
        yearly_pool = 0
        dca_positions = []
        ai_positions = []
        ...
        current_year = None
        last_dca_month = None
        dynamic_chunk_amount = self.dca_amount  # 動態計算的單筆買入金額
        ...
        print(f"[Strategy 1] MODE: NO FILTER (V5 對稱獎勵)")
...
class SharedPoolBacktester:
    ...
    def run(self, df: pd.DataFrame, feature_cols: list) -> dict:
        ...
        print(f"[Strategy 2] MODE: NO FILTER (V5 對稱獎勵)")
